...
namespace OpenAIRequests {
    ...
    public class Requests
    {
        ...
        public IEnumerator PostFormReq<T>(string uri, WWWForm form, System.Action<T> callback, List<(string, string)> headers = null)
        {
            ...
            switch (webRequest.result)
            {
                ...
                case UnityWebRequest.Result.ProtocolError:
                    Debug.LogError("HTTP Error: " + webRequest.error);
                    if (uri.EndsWith("/transcriptions")) {
                      var errJson = JsonUtility.FromJson<ChatGPTResError>(webRequest.downloadHandler.text);
                      Debug.LogError(errJson.error.message);
                    }
                    break;
